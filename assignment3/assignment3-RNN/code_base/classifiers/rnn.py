...
class SentimentAnalysisRNN(object):
    """
    A SentimentAnalysisRNN produces sentiment for a sentence using a recurrent
    neural network.
    ...
    def loss(self, wordvecs, labels, mask):
        ...
        #     (N, T, H).                                                           #
        # (2) Use the temporal affine transformation to map the concatenated       #
        ...
        h1, cache1 = rnn_forward(wordvecs, h0, Wx, Wh, b)
        h2, cache2 = temporal_affine_forward(h1, W_a, b_a)
        h3, cache3 = average_forward(h2, mask)
        out, cache_out = affine_forward(h3, W_fc, b_fc)
        loss, d_out = softmax_loss(out, labels)

        d_out = d_out.reshape(-1, W_fc.shape[1])
        dh3, grads['W_fc'], grads['b_fc'] = affine_backward(d_out, cache_out)
        dh2 = average_backward(dh3, cache3)
        dh1, grads['W_a'], grads['b_a'] = temporal_affine_backward(dh2, cache2)
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh1, cache1)
    ...
    def inference(self, wordvecs, mask):
        ...
        h0 = np.zeros((N, H))
        h1, _ = rnn_forward(wordvecs, h0, Wx, Wh, b)
        h2, _ = temporal_affine_forward(h1, W_a, b_a)
        h3, _ = average_forward(h2, mask)
        out, _ = affine_forward(h3, W_fc, b_fc)
        preds = softmax(out)
